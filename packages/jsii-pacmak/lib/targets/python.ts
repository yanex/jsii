...
import { Translation, Rosetta } from 'jsii-rosetta';
...
class Struct extends BasePythonClassType {
  ...
  private get allMembers(): StructField[] {
    return this.thisInterface.allProperties.map(x => new StructField(this.generator, propertySpec(x)));
  }
...
class Package {
  ...
  public write(code: CodeMaker, resolver: TypeResolver) {
    ...
    if (this.metadata.readme) {
      code.line(this.generator.convertMarkdown(this.metadata.readme.markdown));
    }
...
class PythonGenerator extends Generator {
  ...
  public convertExample(example: string): string {
    const translated = this.rosetta.translateSnippet(example, 'python');
