...
export class JavaBuilder extends AbstractPomBasedTargetBuilder {
  public readonly targetName = 'java';
  public readonly targetConstructor = Java;
...
class JavaGenerator extends Generator {
  ...
  protected onInitializer(cls: spec.ClassType, method: spec.Method) {
    ...
    this.code.line(`super(${JvmAbi.jsiiObject.initializationMode}.JSII);`);
  ...
  private emitStaticInitializer(cls: spec.ClassType) {
    ...
    for (const prop of consts) {
      ...
      this.code.line(`${constName} = ${JvmAbi.jsiiObject.staticGet}(${javaClass}.class, "${prop.name}", ${propClass}.class);`);
    }
  ...
  private emitProperty(cls: spec.Type, prop: spec.Property, includeGetter = true, overrides = !!prop.overrides) {
    ...
    if (includeGetter) {
      ...
      let statement;
      if (prop.static) {
        statement = `${JvmAbi.jsiiObject.type}.jsiiStaticGet(${javaClass}.class, `;
      } else {
        statement = `this.${JvmAbi.jsiiObject.getMethod}(`;
      }
    ...
    if (!prop.immutable) {
      for (const type of setterTypes) {
        ...
        if (prop.static) {
          statement += `${JvmAbi.jsiiObject.staticGet}(${javaClass}.class, `;
        } else {
  ...
  private emitProxy(ifc: spec.InterfaceType | spec.ClassType) {
    ...
    const suffix = ifc.kind === spec.TypeKind.Interface
      ? `extends ${JvmAbi.jsiiObject.type} implements ${this.toNativeFqn(ifc.fqn)}`
      : `extends ${this.toNativeFqn(ifc.fqn)}`;

    this.code.openBlock(`final static class ${name} ${suffix}`);
    this.code.openBlock(`protected ${name}(final ${JvmAbi.jsiiObject.ref} objRef)`);
    this.code.line(`super(${JvmAbi.jsiiObject.initializationMode}.JSII);`);
  ...
  private emitStabilityAnnotations(entity: spec.Documentable) {
    ...
    if (entity.docs.stability) {
      ...
      const levelName = JvmAbi.getStabilityLevelName(entity.docs.stability)
      const levelConst = `${annotationType}.Level.${levelName}`
      this.code.line(`${annotationType}(${levelConst})`)
    }
  }
  ...
  private emitDataType(ifc: spec.InterfaceType) {
    ...
    this.code.line(' */');
    this.emitStabilityAnnotations(ifc);
    this.code.openBlock(`final class ${INTERFACE_PROXY_CLASS_NAME} extends ${JvmAbi.jsiiObject.type} implements ${ifc.name}`);
    ...
    this.code.line(' */');
    this.code.openBlock(`protected ${INTERFACE_PROXY_CLASS_NAME}(final ${JvmAbi.jsiiObject.ref} objRef)`);
    this.code.line(`super(${JvmAbi.jsiiObject.initializationMode}.JSII);`);
    this.code.line('this.setObjRef(objRef);');
    props.forEach(prop => this.code.line(`this.${prop.fieldName} = this.${JvmAbi.jsiiObject.getMethod}("${prop.jsiiName}", ${prop.fieldJavaClass});`));
    this.code.closeBlock();
    ...
    this.code.line();
    this.code.line('@Override');
    this.code.openBlock(`public ${JvmAbi.jackson.jsonNode} ${JvmAbi.jsiiObjectMapper.toJsonMethod}()`);
    this.code.line(`${JvmAbi.jackson.objectMapper} om = ${JvmAbi.jsiiObjectMapper.instance};`);
    this.code.line(`${JvmAbi.jackson.objectNode} obj = ${JvmAbi.jackson.newObjectNode}();`);
  ...
  private renderMethodCall(cls: spec.TypeReference, method: spec.Method, async: boolean) {
    ...
    if (method.static) {
      const javaClass = this.toJavaType(cls);
      statement += `${JvmAbi.jsiiObject.staticCall}(${javaClass}.class, `;
    } else {
      if (async) {
        statement += `this.${JvmAbi.jsiiObject.asyncCallMethod}(`;
      } else {
        statement += `this.${JvmAbi.jsiiObject.callMethod}(`;
      }
    }
  ...
  private emitJsiiInitializers(cls: spec.ClassType) {
    this.code.line();
    this.code.openBlock(`protected ${cls.name}(final ${JvmAbi.jsiiObject.ref} objRef)`);
    this.code.line('super(objRef);');
    this.code.closeBlock();

    this.code.line();
    this.code.openBlock(`protected ${cls.name}(final ${JvmAbi.jsiiObject.initializationMode} initializationMode)`);
